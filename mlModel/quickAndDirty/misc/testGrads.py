...
x = tf.Variable(np.random.random(dims[1]), dtype=dtype)
w = tf.Variable(np.random.random(dims), dtype=dtype)
y = tf.nn.sigmoid(tf_mat_vec_dot(w , x)) 
gradW = jacobian(y, w)
